<?php
// SET HEADER
header("Access-Control-Allow-Orgigin: *");
header("Access-Control-Allow-Headers: access");
header("Access-Control-Allow-Methods: PUT");
header("Content-Type: application/json; charset=UTF-8");
header("Access-Control-Allow-Headers: Content-Type; Access-Control-Allow-Headers, Authorization, X-Requested-With");

// INCLUSION D LA BASE DE DATOS Y CREACION DEL OBJETO DE CONEXION
require 'config.php';
...
if(isset($data->id)){
    ...
    if($get_stmt->rowCount() > 0){
        ...
        $post_nota1 = isset($data->curso) ? $data->nota1 : $row['nota1'];
    ...
    }//CIERRE CORCHETE SEGUNDO IF 
    else{
        $msg['message'] = 'ID invalido';
    }
    //SALIDA EN FORMATO JSON
    echo json_encode($msg);
}//cierra primer corchete if
?>